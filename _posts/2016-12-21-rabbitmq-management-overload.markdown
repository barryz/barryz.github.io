---
layout:     post
title:      "RabbitMQ management DB 过载问题"
subtitle:   "高级篇"
date:       2016-12-21 12:14:48
author:     Barryz
...
> 转载自： 飞哥的blog: https://github.com/moooofly/MarkSomethingDown/blob/master/RabbitMQ/
...
说明：
- 上述数据是在线上环境中、高峰时段抓取的（9月18日）；
- 上述数据均为某个时刻的状态，即状态抽样值；
- 积压消息中绝大部份由 **channel_closed** 、**channel_created** 和 **channel_stats** 构成；而发送 channel_stats 的场景只有两种：a) 基于定时器，默认每隔 5000ms 发送一次；b) 只要存在 channel 的创建和销毁就发送一次；由此可知，统计事件中的 90％ 都和 channel 的创建和销毁有关！！
